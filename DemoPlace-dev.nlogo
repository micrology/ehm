...
;;;  version 1.64 Ric/CE Feb 2023      Implement realtors in a quadrant and shock as a button
;;;  version 1.65 NG  22 Feb 2023      Tidy code
...
globals [
  ...
  initialVacancyRate  ; proportion of empty houses at start
                      ;nRealtors           ; number of realtors (now a slider)
...
to setup
  ...
  print (list "Using Scenario: " scenario)
  ...
  create-realtors nRealtors [
    ...
    ;;; group the relators
    ifelse grouped [
      ;; put the realtors only in the first quadrant
      setxy random max-pxcor random max-pycor
    ][
      ;; distribute the realtors randomly throughtout the town
  ...
  let houses-with-sale-price houses with [ sale-price > 0 ]
  if any? houses-with-sale-price [
  ...
  ask houses [
    ...
    if quality > 3 [set quality 3]
    if quality < 0.3 [set quality 0.3]
  ]
...
to assign-income    ;; owner procedure
  ...
  while [ income < MeanIncome / 2 ] [
    set income (MeanIncome * lambda / alpha ) * (random-gamma alpha lambda) * (1 + (Inflation / (TicksPerYear * 100)) ) ^ ticks
  ]
...
to build-house     ;; observer procedure
  ...
  create-houses 1 [
    ...
    ;  presence of a house
    ; TODO find out the point of this switch
    if not ShowHouses [ hide-turtle ]  ;; temp hold
...
to go
  ...
  if DoShock [
    if ticks = 200 [
      print "**** There has been a shock and parameters have changed!*****"
      if scenario = "ltv"  [ set MaxLoanToValue 60 ]
      if scenario = "ratefall" [ set InterestRate 3 ]
      if scenario = "influx" [ set EntryRate 10 ]
      if scenario = "poorentrants" [ set MeanIncome 24000 ]
...
to step
  ...
  if not ShowHouses [ paint-houses ]
  ...
  ; since we don't model household budgets, add the interest due to the mortgage outstanding
  ; and reduce the mortgage by thecapital repayment
...
to-report valuation [ property ]    ;; realtor procedure
  ...
  ; TODO Justify this magic number
  ;                0.9
  1
...
to do-plots
  ...
  set-current-plot "Gini House Sale Price by Area"
  set-current-plot-pen "area-BL"
  if any? houses-sold [ plot gini-index [ sale-price ] of houses with [area = 1]  ]
  set-current-plot-pen "area-TL"
  if any? houses-sold [ plot gini-index [ sale-price ] of houses with [area = 2] ]
  set-current-plot-pen "area-BR"
  if any? houses-sold [ plot gini-index [ sale-price ] of houses with [area = 3] ]
  set-current-plot-pen "area-TR"
  if any? houses-sold [ plot gini-index [ sale-price ] of houses with [area = 4] ]

  set-current-plot "Gini Income by Area"
  ...
  set-current-plot-pen "Incomes-BR"
  if any? owners [ plot gini-index [ income ] of owners with [ house-area = 3 ]]
  set-current-plot-pen "Incomes-TR"
  if any? owners [ plot gini-index [ income ] of owners with [ house-area = 4 ]]


  set-current-plot "Median House Sale Price by Area"
  set-current-plot-pen "area-BL"
  if any? houses [ plot median [ selling-price ] of houses-sold with [the-area = 1] ]
  set-current-plot-pen "area-TL"
  if any? houses [ plot median [ selling-price ] of houses-sold with [the-area = 2] ]
  set-current-plot-pen "area-BR"
  if any? houses [ plot median [ selling-price ] of houses-sold with [the-area = 3] ]
  set-current-plot-pen "area-TR"
  if any? houses [ plot median [ selling-price ] of houses-sold with [the-area = 4] ]


  set-current-plot "Median Income by Area"
  set-current-plot-pen "Incomes-BL"
  if any? owners [ plot mean [ income ] of owners with [ house-area = 1 ]]
  set-current-plot-pen "Incomes-TL"
  if any? owners [ plot mean [ income ] of owners with [ house-area = 2 ]]
  set-current-plot-pen "Incomes-BR"
  if any? owners [ plot mean [ income ] of owners with [ house-area =  3 ]]
  set-current-plot-pen "Incomes-TR"
  if any? owners [ plot mean [ income ] of owners with [ house-area = 4 ]]

  set-current-plot "Median Income"
  set-current-plot-pen "mean"
  if any? owners [ plot mean [ income ] of owners ]
  set-current-plot-pen "median"
  if any? owners [ plot median [ income ] of owners ]
...
168
36
PwC Housing Market model version 1.6 (dev)
11
104.0
1
...
1460
662
Apply Scenario change at step 200
12
0.0
1
...
PENS
"Incomes-BL" 1.0 0 -2674135 true "" ""
"Incomes-TL" 1.0 0 -14439633 true "" ""
"Incomes-BR" 1.0 0 -14454117 true "" ""
"Incomes-TR" 1.0 0 -10263788 true "" ""
...
1436
492
Median House Sale Price by Area
NIL
£
0.0
10.0
...
PENS
"area-BL" 1.0 0 -2674135 true "" ""
"area-TL" 1.0 0 -14439633 true "" ""
"area-BR" 1.0 0 -13791810 true "" ""
"area-TR" 1.0 0 -7500403 true "" ""
